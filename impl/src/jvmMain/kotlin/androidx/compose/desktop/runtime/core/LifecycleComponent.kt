 ...
 * 如果调用attach时传入了parentLifecycle,则会在parentLifecycle走到onDestroy时触发状态保存
 * 如果未传入parentLifecycle,则需要手动调用screen.onDestroy()以保存状态
 * ```
 */
abstract class LifecycleComponent() :
    ...
    val parentLifecycleObserver = object : LifecycleEventObserver {
     ...
     * @param parentLifecycle 父组件生命周期(可选).用于将当前组件实例生命周期关联到父组件的生命周期
     */
    open fun attach(token: Token? = null, parentLifecycle: Lifecycle? = null) {
        this.token = token
        ...
        parentLifecycle?.let {
            ...
            it.addObserver(parentLifecycleObserver)
        }
    }
...
//        logger.info("保存状态，uuid:$uuid")
     ...
     * 断开与父组件的生命周期链接
     */
    protected fun deSyncParentLifecycle() {
        //移除对于parent的生命周期监听
        parentLifecycle.get()?.removeObserver(parentLifecycleObserver)
